...
#
# USAGE:
#   ./scripts/start.sh          # Run in foreground (shows live logs, Ctrl+C stops everything)
#   ./scripts/start.sh -d       # Run in background (detached mode)
#   ./scripts/start.sh -f       # Follow logs (use after starting with -d)
#
# DESCRIPTION:
#   This script starts all required services for the ANPR Client Backend:
#   1. Docker services (PostgreSQL + Redis)
#   2. Celery Worker (for background task processing)
#   3. Flower (Celery monitoring dashboard)
#   4. FastAPI (main API server)
#
# NOTES:
#   - Script must be run from project root: ./scripts/start.sh
#   - Automatically stops any existing processes before starting
#   - Creates logs in logs/ directory
#   - Press Ctrl+C in foreground mode to stop all services
#   - Use ./scripts/stop.sh to stop services in daemon mode
#
# ============================================
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: ./scripts/start.sh [-d|--daemon] [-f|--follow]"
            exit 1
            ;;
    esac
done
...
if [ "$DAEMON_MODE" = true ]; then
    ...
    echo ""
    echo "Commands:"
    echo "  ./scripts/start.sh -f     # Follow logs"
